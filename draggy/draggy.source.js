function initDraggy(settings) {
    ...
    class Rectangle {
        ...
        intersects(rect) {
            return !(
                ((this.obj.position().top + this.obj.height()) < (rect.obj.position().top)) ||
                (this.obj.position().top > (rect.obj.position().top + rect.obj.height())) ||
                ((this.obj.position().left + this.obj.width()) < rect.obj.position().left) ||
                (this.obj.position().left > (rect.obj.position().left + rect.obj.width()))
            );
        }
    ...
    function initBoxes() {
        $(".rect").each(function () {
            var dbx = new Rectangle(new Vector2($(this).position().left, $(this).position().top), $(this), $(this).attr("uid"));
    ...
    function ghostDraggable(uid) {
        ...
        newGhostObj = dragBoxes[uid].obj.clone().appendTo("body");
        ...
        currentGhost.setPosition(new Vector2(
            dragBoxes[uid].obj.position().left,
            cursorPos.y - dragBoxes[uid].obj.height() / 2));

        startOffset = new Vector2(
            cursorPos.x - currentGhost.obj.position().left,
            cursorPos.y - currentGhost.obj.position().top);
        ...
        //calback
        settings.onEnable(dragBoxes[uid].obj);
    }
    ...
    function moveGhost() {
        ...
        settings.onDrag(dragBoxes[currentGhost.uid].obj);
    }
    ...
    function mouseStopped() {
        ...
        if (currentGhost !== null)
            settings.onHalt(dragBoxes[currentGhost.uid].obj);
    }


    $(document).bind("touchend touchcancel mouseup", function () {
        ...
        for (var k in dropBoxes) {
            if (currentGhost.intersects(dropBoxes[k])) {
                dragBoxes[currentGhost.uid].obj.appendTo(dropBoxes[k].obj);

                if (settings.onDrop !== undefined) {
                    //callback
                    settings.onDrop(dragBoxes[currentGhost.uid].obj, dropBoxes[k].obj);
